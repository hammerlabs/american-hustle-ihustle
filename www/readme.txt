* place the contents of the www folder in the web root (you don't want a www folder on the sony server, just the contents)
* the contents of the assets folder can be placed on the CDN (you don't want an assets folder on the sony server, just the contents)
* new images are saved to the www/user_images folder and PHP will need permissions to write files there
* the www/_settings.php file has all teh relevant configuration info centralized in it, but you'll likely only need to change the test and production Tumblr settings for CONSUMER_KEY, CONSUMER_SECRET, OAUTH_TOKEN, OAUTH_SECRET
...
* installing this on the Tumblr theme requires the following steps:
  ...
  * in the content region, click the HTML button and paste this: <iframe height="460" id="ihustle_frame" scrolling="no" src="http://ah.hammerlabs.com/www/" width="680"></iframe>
  * change the URL in the src attribute of the iframe tag to point whereever you have the site installed.. I think its something like "http://stage.sonypictures.com/movies/americanhustle/tumblr/hustle/"
  ...
  * Add OAuth Consumer Key and Secret Key to www/_settings.php variables
  ...
  * Add new tokens to the variables to www/_settings.php
